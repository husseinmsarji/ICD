...
A single, schema-validated YAML interface definition file is the
...
(`icdgen/fields.py`). The JSON Schema, the serializer, the parser, the
...
2. **C header** — `struct` definitions and `#define` macros per packet/signal
   (min/max/scale/offset/rate). MISRA C:2012-oriented: fixed-width integer
   types, fully parenthesized macros, sanitized macro names.
...
Python 3.10+. No cloud dependencies — fully offline. Dependencies are
exact-pinned to protect byte-determinism. Runs on Windows and macOS.

## Usage

```bash
# Validate only (exit 1 on any schema/semantic error, with a line reference)
python -m icdgen validate examples/icd_evtol_revC.yaml

# Generate all artifacts into ./out
python -m icdgen generate examples/icd_evtol_revC.yaml -o out

# Generate a subset
python -m icdgen generate input.yaml -o out -f header trace-csv pdf
...
- **Completeness, no silent failures.** Missing required fields raise a
  validation error with a file/line reference, never a default-filled silent
  pass.
...
- **DO-330 tool-qualification evidence.** Generated documents embed a tool
  ...
  version, input hash, schema version, artifacts, the generated-schema hash,
  the active template directory, and a per-template hash manifest).

## Schema and extensibility

The input schema is **versioned** via a `schemaVersion` key pinned to `1.x`.
Extensibility contract:

- Additive-only within a major version: new **optional** keys may appear in a
  minor revision; required fields are only added in a major bump, so existing
  files never break.
- An `extensions` mapping lets a program carry custom payload without a schema
  change.

Validation uses a JSON Schema generated from the field registry
(`schema_gen.py`) and applied to the parsed YAML, so the schema cannot drift
from the registry. Canonical YAML is produced by a hand-rolled, byte-stable
emitter (`serializer.to_yaml`): fixed key order, quoted strings, minimal
optional fields. Set `ICDGEN_TEMPLATE_DIR` to point the Jinja templates at a
program-specific copy (recorded in `run.log` under provenance).
...
The spec bundles the Jinja templates as data; `resources.py`
...
icdgen/
  ...
  loader.py              YAML parse + JSON Schema validation; line refs; warnings channel
  serializer.py          IcdModel -> canonical YAML (deterministic emitter)
...
examples/                icd_evtol_revA/B/C.yaml (three revisions of ICD-EVTOL-AVS-200)
...
Covers validation success/failure, line references, enum and
